...
// --- COMPOSANT DE RENDU DES TORSEURS (S2I Style) ---
const TorsorDisplay = ({ title, col1, col2, point, characteristic }) => (
  <div className="p-5 bg-indigo-50/30 border border-indigo-100 rounded-2xl shadow-inner">
    ...
    <div className="flex flex-col items-center">
      ...
      {/* Caractéristiques géométriques sous le torseur */}
      {characteristic && (
        <div className="mt-5 pt-3 border-t border-indigo-100 w-full text-center">
...
const CULTURE_DATA = {
  Actionneurs: {
    ...
    items: [
      { 
        ...
        info: "Idéal pour l'asservissement de vitesse.",
        details: "Convertisseur réversible géré par un couplage électromagnétique linéaire.",
        principe: "Force de Laplace : interaction courant/champ magnétique.",
        entrees_sorties: { entrée: "Tension, Courant", sortie: "Vitesse, Couple" },
        caracs: ["C = Kc · I", "E = Ke · Ω", "U = E + R · I"],
        links: [{ label: "Fiche MCC", url: "#" }],
        img: "https://images.unsplash.com/photo-159742324403d-11474e86a6fb?auto=format&fit=crop&q=80&w=600"
      }
    ]
  },
  Transmetteurs: {
  ...
  Capteurs: {
    ...
    description: "Ils informent la partie commande sur l'état du système.",
    items: [
      { 
        ...
        entrees_sorties: { entrée: "Position θ", sortie: "Impulsions (Numérique)" },
        caracs: ["Résolution (pts/tr)"],
        links: [{ label: "Doc technique", url: "#" }],
  ...
  Liaisons: {
    ...
    description: "Définissent les mobilités relatives et les efforts transmissibles entre deux solides.",
    items: [
      { 
        name: "Liaison Pivot", 
        ...
        details: "Liaison de base interdisant toute translation et n'autorisant qu'une seule rotation autour de son axe.",
      ...
      { 
        ...
        details: "Liaison n'autorisant qu'une unique translation suivant un axe donné.",
        ex_labo: ["Vérin Pilote Auto"],
...
const SYSTEMS = [
  { id: 'maxpid', name: 'Système Maxpid', category: 'Robotique', description: 'Bras manipulateur asservi.', image: 'https://images.unsplash.com/photo-1581091226825-a6a2a5aee158?auto=format&fit=crop&q=80&w=400', links: { dt: "#", dr: "#" } }
];
                    ...
                    {selectedCulture === 'Liaisons' ? (
                      <div className="bg-white rounded-[1.5rem] p-8 text-gray-900 shadow-2xl">
                        <div className="flex justify-between items-start mb-10">
                          <div className="text-left">
                            <h3 className="text-4xl font-black uppercase text-gray-900 tracking-tight">{currentItem.name}</h3>
                        ...
                        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                          <div className="space-y-6 text-left">
                            ...
                            <div className="bg-gray-50 p-5 rounded-2xl border border-gray-100">
                              <h4 className="text-[10px] font-black uppercase tracking-widest text-gray-400 mb-4 flex items-center gap-2"><Move size={14}/> Degrés de liberté</h4>
                            ...
                            <div>
                               <h4 className="text-[10px] font-black uppercase tracking-widest text-gray-400 mb-3">Exemples Labo :</h4>
                               <div className="flex flex-wrap gap-2">
                                 {currentItem.ex_labo.map((ex, i) => <span key={i} className="px-2 py-1 bg-gray-900 text-white rounded text-[9px] font-bold uppercase tracking-tighter">{ex}</span>)}
                               </div>
                            </div>
                          </div>

                          <div className="space-y-6 text-left">
                            <h4 className="text-[10px] font-black uppercase tracking-widest text-indigo-600 mb-2 flex items-center gap-2"><Layout size={14}/> Schématisations</h4>
                            <div className="border border-gray-100 rounded-2xl p-4 bg-gray-50 flex flex-col items-center justify-center min-h-[140px] text-center">
                              <span className="text-[9px] font-bold text-gray-400 uppercase mb-3 italic tracking-tight">{currentItem.schematisation.ortho}</span>
                              <div className="w-32 h-20 bg-white border border-dashed border-gray-200 rounded-xl flex items-center justify-center text-[9px] text-gray-300 uppercase font-black">2D Symbol</div>
                            </div>
                            <div className="border border-gray-100 rounded-2xl p-4 bg-gray-50 flex flex-col items-center justify-center min-h-[140px] text-center">
                              <span className="text-[9px] font-bold text-gray-400 uppercase mb-3 italic tracking-tight">{currentItem.schematisation.persp}</span>
                              <div className="w-32 h-20 bg-white border border-dashed border-gray-200 rounded-xl flex items-center justify-center text-[9px] text-gray-300 uppercase font-black">3D Symbol</div>
                            </div>
                          </div>

                          <div className="space-y-6 text-left">
                            <h4 className="text-[10px] font-black uppercase tracking-widest text-indigo-600 mb-2 flex items-center gap-2"><FileText size={14}/> Modélisation Torseurs</h4>
                            
                            <TorsorDisplay 
                              title="Torseur Cinématique {V}" 
                              col1={currentItem.tors_cin.col1} 
                              col2={currentItem.tors_cin.col2} 
                              point={currentItem.tors_cin.point}
                              characteristic={currentItem.tors_cin.char}
                            />

                            <TorsorDisplay 
                              title="Torseur Statique {T}" 
                              col1={currentItem.tors_stat.col1} 
                              col2={currentItem.tors_stat.col2} 
                              point={currentItem.tors_stat.point}
                              characteristic={currentItem.tors_stat.char}
                            />
                          </div>
                        </div>
                      </div>
                      ...
                      <div className="bg-white rounded-[1.5rem] p-8 text-gray-900 flex flex-col lg:flex-row gap-10 shadow-2xl">
                        <div className="lg:w-1/3 space-y-6 text-left">
                          ...
                          <div className="space-y-4">
                            <h4 className="flex items-center gap-2 text-[10px] font-black uppercase tracking-widest text-indigo-600 border-b border-gray-50 pb-2"><Info size={14}/> Analyse Fonctionnelle</h4>
                        ...
                        <div className="lg:w-2/3 space-y-8 text-left">
                          <div className="flex justify-between items-start">
                            <h3 className="text-3xl font-black uppercase text-gray-900 tracking-tight">{currentItem.name}</h3>
                            <button onClick={() => setSelectedItemName(null)} className="p-2 hover:bg-gray-100 rounded-full transition-all"><X size={20} className="text-gray-400" /></button>
                          </div>
                          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                            <div className="space-y-6">
                              <div>
                                <h4 className="text-[10px] font-black uppercase tracking-widest text-gray-400 mb-3 flex items-center gap-2"><Activity size={14}/> Principe Physique</h4>
                              ...
                              <div>
                                <h4 className="text-[10px] font-black uppercase tracking-widest text-gray-400 mb-3 flex items-center gap-2"><Layout size={14}/> Flux d'Information</h4>
                            ...
                            <div className="space-y-6">
                              ...
                              <div className="space-y-2">
                                {currentItem.caracs?.map((c, idx) => (
                                  <div key={idx} className="bg-white p-4 rounded-xl border border-gray-100 shadow-sm flex items-center justify-center font-serif text-sm font-medium italic text-indigo-700">{c}</div>
                                ))}
                              </div>
                            </div>
                          </div>
        ...
        {!selectedCulture && (
          <div className="mt-12 text-left">
            <h2 className="text-2xl font-bold text-gray-900 mb-8 flex items-center gap-2">
              <Cpu className="text-indigo-600" size={24} /> Systèmes disponibles
            </h2>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {SYSTEMS.map((s) => (
                <div key={s.id} className="bg-white rounded-2xl border border-gray-100 overflow-hidden shadow-sm hover:shadow-md hover:border-indigo-200 transition-all group">
                  <div className="h-40 overflow-hidden relative border-b border-gray-50">
                    <img src={s.image} className="w-full h-full object-cover" alt={s.name} />
                    <div className="absolute top-3 left-3">
                      <span className="bg-white/95 text-indigo-600 text-[9px] font-black uppercase tracking-widest px-2.5 py-1 rounded-lg shadow-sm border border-indigo-50">{s.category}</span>
                    </div>
                  </div>
                  <div className="p-5">
                    <h3 className="text-lg font-bold text-gray-900 mb-5 group-hover:text-indigo-600 transition-colors">{s.name}</h3>
